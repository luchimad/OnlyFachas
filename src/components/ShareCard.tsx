...
const GlowText: React.FC<{ children: React.ReactNode; className?: string; color: 'pink' | 'yellow'; style?: React.CSSProperties }> = ({ children, className, color, style }) => {
  const colorClasses = {
  ...
  return <span className={`${colorClasses[color]} ${className}`} style={style}>{children}</span>;
};
